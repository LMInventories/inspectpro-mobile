...
const HELP_CONTENT: Record<string, { cmd: string; note: string }[]> = {
  ...
  subs: [
    { cmd: 'Walls. White emulsion. In good order. Add sub item. Light scuffing to base of wall.',
      note: 'Say "add sub item" (or "add sub-item", "sub-item", "next sub item" — any variation) to close the current element and start a new one. The AI fills each separately.' },
    ...
    { cmd: 'Return to Walls, add sub item, light scuffing to base of wall',
      note: 'To add a sub-item to an item you\'ve already transcribed, say "Return to [item name], add sub item [description and condition]".' },
  ...
  skip: [
    { cmd: 'Windows & Frames … please delete',
      note: 'By Room: say "[item name] please delete" immediately after the item name to remove it from the report. This is the ONLY phrase that deletes an item — nothing else does.' },
    { cmd: 'Please delete (Instant mode)',
      note: 'Instant mode: start a per-item recording and just say "please delete" — the item is already known and will be deleted.' },
    { cmd: 'Important: embedded use is kept as dictation',
      note: '"BOSCH black glass hob, model and serial number not seen" is written as-is — "please delete" only fires when it immediately follows the item title with nothing else.' },
    { cmd: 'Items can also be deleted manually',
      note: 'Swipe left on any item in the room list to delete it without dictating.' },
  ],
  return: [
    { cmd: 'Amend Door and Frame description white painted panel door with brass handle',
      note: 'By Room (primary): say "Amend [item name] description [new content]" to overwrite that field. Use "Amend [item name] condition" for condition, or "Amend [item name]" alone to replace both.' },
    { cmd: 'Add to Walls condition light scuffing to base throughout',
      note: 'By Room: "Add to [item name] condition [content]" appends to what\'s already there. "Add to [item name] description" appends to description. "Add to [item name]" appends to both.' },
    { cmd: 'Ceiling. Amend condition. Light scratching.',
      note: 'By Room (alternate): say the item name as a heading, then follow immediately with "amend", "amend condition", "amend description", "add", "add to condition", or "add to description" — works the same as the primary format.' },
    { cmd: 'Return to Door and Frame, amend description, white painted panel door',
      note: 'By Room (also accepted): "Return to [item name], amend description / amend condition / amend / add" all work if you prefer that phrasing.' },
    { cmd: 'Amend … / Add …',
      note: 'Instant mode only: start a per-item recording with just "amend" (overwrite) or "add" (append) — the item is already known from context.' },
    { cmd: 'Amend description … / Amend condition … / Add to description … / Add to condition …',
      note: 'Instant mode: to target one field specifically, say "amend description", "amend condition", "add to description", or "add to condition" before the new content.' },
  ],
  tips: [
    { cmd: 'Say everything — the AI only removes "um" and "er"',
      note: 'The AI takes your words as read. It only strips filler sounds (um, uh, er). Everything else you say will appear in the report, so describe fully and don\'t hold back.' },
